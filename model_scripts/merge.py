...
def LightGBM(X_train, y_train, TEST_MODE = True, RANDOM_SEED = 42):
    if TEST_MODE:
        ...
        tmp_X_train, tmp_X_test, tmp_y_train, tmp_y_test = X_train, X_train, y_trains[i], y_trains[i]
...
def merge_DL(dataset_names, X_trains, y_trains, X_tests, RANDOM_SEED = 42):
    ...
    for i in tqdm(range(len(dataset_names))):
        simpleDL_model = simple_DL(X_train = X_trains[i], y_train = y_trains[i], TEST_MODE = True, RANDOM_SEED = RANDOM_SEED)
        XGB_model = XGBoost(X_train = X_trains[i], y_train = y_trains[i], TEST_MODE = True, RANDOM_SEED = RANDOM_SEED)
        LightGBM_model = LightGBM(X_train = X_trains[i], y_train = y_trains[i], TEST_MODE = True, RANDOM_SEED = RANDOM_SEED)
        ...
        train_tensor = TensorDataset(torch.FloatTensor(pd.DataFrame({
            ...
            'XGB': XGB_y_prob, 
            'LightGBM': LightGBM_y_prob}).values), torch.FloatTensor(tmp_y_train.values))
        train_loader = DataLoader(train_tensor, batch_size = 32, shuffle = True)
        ...
        with torch.no_grad():
            ...
            tmp_y_prob = merge_model(torch.FloatTensor(pd.DataFrame({
                                    'DL': simple_DL_y_prob[:, 0], 
                                    'XGB': XGB_y_prob,
                                    'LightGBM': LightGBM_y_prob}).values)).numpy()
        ...
        del LightGBM_model, XGB_model, merge_model, simpleDL_model
